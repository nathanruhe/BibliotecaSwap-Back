...
async function getUsersWithChats(request, response) {
  ...
  if (!id_user) {
    return response
      .status(400)
      .json({ error: true, message: "ID de usuario no proporcionado" });
  }

  try {
    const [users] = await pool.query(
      `
            SELECT DISTINCT u.id_user, u.name, u.last_name, u.photo, c.id_chat
            FROM chat c
    ...
    if (users.length === 0) {
      return response
        .status(404)
        .json({ error: true, message: "No se encontraron usuarios con chats" });
    }
    ...
    response
      .status(500)
      .json({ error: true, message: "No se pudo buscar usuarios con chats" });
  }
}
...
async function getMessages(request, response) {
  ...
  if (!id_chat) {
    return response
      .status(400)
      .json({ error: true, message: "ID de chat no proporcionado" });
  }
  ...
  try {
    ...
    if (messages.length === 0) {
      return response
        .status(404)
        .json({ error: true, message: "No se encontraron mensajes" });
    }
    ...
    response
      .status(500)
      .json({ error: true, message: "Error al obtener mensajes" });
  }
}
...
async function enviarMensaje(request, response) {
  try {
    ...
    if (!id_user1 || !id_user2 || !emisor || !message) {
      return response
        .status(400)
        .json({ error: true, message: "Faltan datos en la solicitud" });
    }
    ...
    params = [
      id_chat,
      emisor,
      emisor === id_user1 ? id_user2 : id_user1,
      message,
    ];
    ...
    sql = `
            UPDATE chat 
            SET ${emisor === id_user1 ? "noLeido_user2" : "noLeido_user1"} = ${
      emisor === id_user1 ? "noLeido_user2" : "noLeido_user1"
    } + 1
            WHERE id_chat = ?
        `;
    params = [id_chat];
    ...
    response
      .status(200)
      .json({ error: false, message: "Mensaje enviado con éxito" });
  } catch (error) {
    console.error("Error enviando mensaje:", error);
    response
      .status(500)
      .json({ error: true, message: "Error enviando mensaje" });
  }
}
...
async function resetUnreadMessages(request, response) {
  ...
  if (!id_chat || !userId) {
    return response
      ...
      .json({
        error: true,
        message: "ID de chat o ID de usuario no proporcionado",
      });
  }
  ...
  try {
    ...
    response
      ...
      .json({
        error: true,
        message: "No se pudieron resetear los mensajes no leídos",
      });
  }
}
  ...
  try {
    ...
    if (chats.length === 0) {
      return res
        ...
        .json({
          error: true,
          message: "No se encontraron chats para el usuario especificado",
        });
    }
    ...
    res
      .status(200)
      .json({ error: false, message: "Chats eliminados correctamente" });
  } catch (error) {
